...
<template>
  <section id="experience" class="max-w-7xl mx-auto px-6 py-24 md:py-32 relative">
    <div class="mb-20 text-center md:text-left">
      ...
      <h2 class="font-serif text-6xl md:text-8xl tracking-tighter">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-16 items-start">
      <div class="lg:col-span-7 relative">
        ...
        <div class="space-y-6">
          <div v-for="(job, index) in jobs" :key="job.id" 
            ...
            <div class="glass-panel p-6 md:p-8 rounded-[2.5rem] border border-white/5 hover:border-accent/30 transition-all duration-700 relative overflow-hidden">
              <div class="absolute inset-0 opacity-[0.02] pointer-events-none noise-bg"></div>
              
              <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 relative z-10">
                <div class="flex items-center gap-6">
                  <div class="w-16 h-16 shrink-0 rounded-2xl bg-zinc-950 border border-white/10 flex items-center justify-center p-3 group-hover:rotate-6 transition-transform duration-500">
                  ...
                  <div>
                    <h3 class="text-xl md:text-2xl font-serif text-zinc-100 group-hover:text-accent transition-colors leading-none mb-2">
                      {{ job.position }}
                    </h3>
                    <div class="flex items-center gap-2">
                      <span class="text-zinc-500 text-[10px] uppercase tracking-widest font-bold">{{ job.companyName }}</span>
                      <span class="w-1 h-1 rounded-full bg-white/10"></span>
                      <span class="text-zinc-500 text-[9px] font-mono tracking-tighter">{{ job.startDate }} — {{ job.endDate }}</span>
                    </div>
                  </div>
                </div>

                <a :href="job.link" target="_blank" 
                   class="w-12 h-12 rounded-full border border-white/10 flex items-center justify-center hover:bg-accent hover:text-black transition-all group-hover:border-accent/50">
      ...
      <div class="lg:col-span-5 space-y-6 lg:sticky lg:top-32">
        <div class="glass-panel rounded-[3rem] p-10 border border-white/10 relative overflow-hidden shadow-2xl shadow-black/50">
          ...
          <div class="flex justify-between items-start mb-12 relative z-10">
            <div>
              ...
              <h4 class="font-serif text-3xl">Impact Analysis</h4>
            </div>
          ...
          <div class="space-y-8 relative z-10">
            <div v-for="stat in metrics" :key="stat.label" class="space-y-3 group/stat">
              <div class="flex justify-between items-end">
                <span class="text-[10px] uppercase tracking-[0.3em] text-zinc-500 font-bold group-hover/stat:text-white transition-colors">{{ stat.label }}</span>
          ...
          <div class="mt-12 pt-8 border-t border-white/5 relative z-10">
            <p class="text-[10px] text-zinc-500 leading-relaxed uppercase tracking-[0.2em]">
              Data synchronized with <span class="text-white">full-stack logic</span> & <span class="text-white">active security protocols</span>.
            </p>
          </div>
        </div>

        <div class="glass-panel rounded-4xl p-6 border border-white/5 flex items-center gap-6 group hover:border-accent/20 transition-all">
          <div class="w-12 h-12 rounded-xl bg-accent/5 flex items-center justify-center text-accent text-xl group-hover:scale-110 transition-transform">✦</div>
