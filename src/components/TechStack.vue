...
<template>
  <section id="stack" class="max-w-7xl mx-auto px-4 sm:px-6 py-16 md:py-32 scroll-mt-32">
    <div class="flex flex-col md:flex-row md:items-end justify-between mb-10 md:mb-20 gap-6 md:gap-8">
      <div class="space-y-2 md:space-y-4">
        <span class="text-accent text-[10px] uppercase tracking-[0.6em] font-bold block opacity-80">Capabilities</span>
        <h2 class="font-serif text-4xl sm:text-6xl md:text-8xl tracking-tighter leading-none">
          Engine<span class="italic text-zinc-600">.</span>
        </h2>
      </div>
      <div class="border-l border-white/10 pl-6 py-2 max-w-sm">
        <p class="text-zinc-400 text-[10px] md:text-[11px] leading-relaxed uppercase tracking-[0.25em]">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 lg:grid-rows-6 gap-4 auto-rows-[minmax(180px,auto)] md:auto-rows-[minmax(200px,auto)]">
      <div 
        v-for="(cat, index) in categories.slice(0, 5)" 
        :key="cat.title"
        :class="[getGridClass(index)]"
        class="group relative glass-panel p-6 md:p-8 rounded-[24px] md:rounded-4xl border border-white/5 hover:border-accent/30 transition-all duration-700 flex flex-col justify-between overflow-hidden"
      >
        ...
        <div class="flex justify-between items-start mb-8 md:mb-10 relative z-10">
          <div class="space-y-2">
            <h3 class="text-[9px] md:text-[10px] uppercase tracking-[0.4em] text-zinc-400 font-bold group-hover:text-accent transition-colors">
              {{ cat.title }}
            </h3>
        ...
        <div class="grid grid-cols-4 sm:grid-cols-5 md:grid-cols-4 lg:grid-cols-4 xl:grid-cols-5 gap-3 md:gap-4 relative z-10">
          <div v-for="tech in cat.icons" :key="tech.name" class="relative group/icon">
            ...
            <a :href="tech.link" target="_blank"
              class="w-10 h-10 md:w-11 md:h-11 rounded-2xl bg-white/3 border border-white/5 flex items-center justify-center p-2 md:p-2.5 hover:border-accent/40 hover:bg-accent/10 transition-all duration-500 group-hover/icon:-translate-y-1.5 shadow-xl shadow-black/20">
